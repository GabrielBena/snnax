...
ForwardFn = Callable[
    [Sequence[eqx.Module], GraphStructure, PRNGKey, Sequence[Array], PyTree],
    ForwardFnOutput,
]
...
class StatefulModel(eqx.Module):
    ...
    def __init__(
        ...
        assert (
            len(self.graph_structure.input_connectivity)
            == self.graph_structure.num_layers
        )
    ...
    def init_state(
        ...
        for layer_id, (layer, key) in enumerate(zip(self.layers, keys)):
            ...
            # If the node is also a input layer, also append external input
            external_inputs = [
                jnp.zeros(in_shape_0[id]) for id in struct.input_layer_ids[layer_id]
            ]
            inputs += external_inputs
    ...
    def __call__(
        self, input_states: Sequence[Array], input_batch, key: PRNGKey, burnin: int = 0
    ) -> Tuple:
        # Partial initialization of the forward function
        forward_fn = ft.partial(self.forward_fn, self.layers, self.graph_structure, key)

        if burnin > 0:
            new_states, new_outs = self.loop_fn(
                forward_fn, input_states, input_batch[:burnin]
            )

            # Performes the actual BPTT when differentiated
            new_states, new_outs = self.loop_fn(
                forward_fn, jax.lax.stop_gradient(new_states), input_batch[burnin:]
            )
        else:
            new_states, new_outs = self.loop_fn(forward_fn, input_states, input_batch)
        return new_states, new_outs
